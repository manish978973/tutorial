...
var (
 ...
 dpath := "/run/shm/dispatcher/default.sock"
  ...
  logerror(ef)
receivePacketBuffer := make([]byte, 40000)  //Intiating a dynamic array of respective size

  for {
  		b, clientAddr, ef := scionconnection.ReadFrom(receivePacketBuffer) //decoding the value from buffer
  	 logerror(ef)

  		// Packet received, send back response to same client
  		a := binary.PutVarint(receivePacketBuffer[b:], time.Now().UnixNano()) //encoding a value to buffer
